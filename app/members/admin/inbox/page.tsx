...
export default function AdminInboxPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const myRole = roleRow?.role ?? "member";
      setRole(myRole);
      ...
      // ✅ 3. Pull requests (admin visibility)
      const { data: reqs, error } = await supabase
        .from("requests")
        .select(
          "id, created_at, created_by, category, subject, status, visibility, last_updated, urgent_note"
        )
        .eq("visibility", "admin")
      ...
      // ✅ 4. Fetch profile labels
      const ids = Array.from(
        new Set(list.map((x) => x.created_by))
      ).filter(Boolean);
